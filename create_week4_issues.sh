#!/usr/bin/env bash
# Create Week 4 Milestone + 10 Issues in bancaemdia-api repo
# Usage: chmod +x create_week4_issues.sh && ./create_week4_issues.sh
...
MILESTONE_NUMBER=$(gh api repos/$REPO/milestones \
  -f title="$MILESTONE" \
  -f description="Replay CLI, idempotency tests, edge cases, failure injection, runbooks, k6 load test, AWS Terraform, CD pipeline, production hardening, final validation." \
  -f due_on="$(date -d '+28 days' -u +%Y-%m-%dT%H:%M:%SZ)" \
  --jq '.number')
...
# Issue 1: Replay & Reprojeção CLI
create_issue \
  "[Week 4] Replay & Reprojeção CLI — Full Rebuild from Event Log" \
...
# Issue 2: Idempotency Test Suite
create_issue \
  "[Week 4] Idempotency Test Suite — Exhaustive Duplicate Testing" \
...
## Acceptance
\`pytest tests/idempotency/ -v\` passes; zero duplicate rows in any scenario"

# Issue 3: Edge Cases — Financial & Temporal Correctness
create_issue \
  "[Week 4] Edge Cases — Financial & Temporal Correctness" \
...
- [ ] **Freebet**: \`stake_centavos=0\`, \`valor_aposta_centavos=10000\` (R$ 100)
  - Green: \`retorno = valor_aposta * (odd - 1)\` (profit only)
  - Red: \`retorno = 0\` (no loss)
...
## Acceptance
All edge case tests pass; centavo-exact match expected values"

# Issue 4: Failure Injection & Chaos Testing
create_issue \
  "[Week 4] Failure Injection & Chaos Testing" \
...
## Acceptance
All chaos scenarios tested; \`conferir_numeros.py\` passes after each"

# Issue 5: Runbooks — Operations Documentation
create_issue \
  "[Week 4] Runbooks — Operations Documentation" \
...
## Acceptance
All 5 runbooks written, < 2 pages each, linked in \`RUNBOOKS.md\`"

# Issue 6: Load Test — k6 Script + CI Integration
create_issue \
  "[Week 4] Load Test — k6 Script + CI Integration" \
...
## Acceptance
\`k6 run k6/load-test.js\` passes all thresholds on staging"

# Issue 7: AWS Production Staging Provisioning (Terraform)
create_issue \
  "[Week 4] AWS Production Staging Provisioning (Terraform)" \
...
## Acceptance
\`terraform apply\` creates staging env; API deploys and passes health checks"

# Issue 8: CD Pipeline — Build → Staging → Canary → Production
create_issue \
  "[Week 4] CD Pipeline — Build → Staging → Canary → Production" \
...
## Acceptance
Push to main → staging deployed in < 10 min; canary → prod in < 30 min; rollback < 5 min"

# Issue 9: Production Hardening — Security, Compliance, Cost Controls
create_issue \
  "[Week 4] Production Hardening — Security, Compliance, Cost Controls" \
...
## Acceptance
Security scan passes; secrets audit clean; LGPD endpoints implemented"

# Issue 10: Final Validation — Staging = Production Mirror
create_issue \
  "[Week 4] Final Validation — Staging = Production Mirror" \
...
## Acceptance
All gates pass; ready for production traffic"
