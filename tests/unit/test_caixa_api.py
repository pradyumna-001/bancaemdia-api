...
def _banco(
    *,
    ...
    class ApostaRepo:
        async def list_by_usuario(self, session, usuario_id, **kwargs):
            return list(banco.apostas)
...
def test_balance_groups_history_before_calculating_each_account(monkeypatch) -> None:
    banco = _banco(
        contas=[_conta(10), _conta(20)],
    ...
    observadas: list[tuple[int, list[int | None], list[int | None]]] = []
    saldo_real = rota.saldo_da_conta

    def observar(conta_id, apostas, movimentos, data_corte=None):
        apostas_da_conta = list(apostas)
        movimentos_da_conta = list(movimentos)
        observadas.append((
            conta_id,
            [aposta.conta_casa_id for aposta in apostas_da_conta],
            [movimento.conta_casa_id for movimento in movimentos_da_conta],
        ))
        return saldo_real(conta_id, apostas_da_conta, movimentos_da_conta, data_corte)

    monkeypatch.setattr(rota, "saldo_da_conta", observar)
    cliente = _cliente(monkeypatch, banco)

    resposta = cliente.get("/api/v1/caixa/saldo")

    assert resposta.status_code == 200
    assert observadas == [(10, [10], [10]), (20, [20], [20])]
