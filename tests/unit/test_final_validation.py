...
def test_phase1_gate_uses_local_capacity_and_lightsail_operational_proof(tmp_path: Path) -> None:
    manifest = _manifest(tmp_path)
    manifest["phase"] = "lightsail-phase1"
    manifest["environment"] = "production"
    integrity_path = tmp_path / "integrity.json"
    integrity = json.loads(integrity_path.read_text(encoding="utf-8"))
    integrity.update({"environment": "testing", "mode": "phase1-gate"})
    ...
    load["bancaemdia"]["environment"] = "testing"
    load_path.write_text(json.dumps(load), encoding="utf-8")
    for name in final_validation.PHASE1_CHECKS:
        manifest["checks"][name] = {"passed": True, "evidence": URL}
    manifest["checks"].pop("rds_failover")
    manifest["cost"].update({
        "infrastructure_usd": 0.5,
        "ai_usd": 0.5,
        "usd_to_brl": 5,
        "fx_evidence": URL,
    })
    ...
    manifest["checks"]["restore_drill"]["passed"] = False
    assert "restore_drill" in _failed(manifest, tmp_path)
    manifest["checks"]["restore_drill"]["passed"] = True
    manifest["cost"]["usd_to_brl"] = 8
    assert "custos 24h" in _failed(manifest, tmp_path)
