# Week 2: Async Pipeline — Milestones & GitHub Issues

## Milestone: **Week 2 — Async Pipeline (Celery + Redis + Dual Pools + Extraction)**

**Target Date**: 7 days from Week 1 completion
**Depends on**: Week 1 Foundation complete (models, RLS, domain logic, CI green)
**Success Criteria**:
- [ ] Redis running locally via `docker-compose.yml` (added Week 1)
- [ ] Celery app with 2 queues: `extraction`, `materialization`
- [ ] Extraction workers: OCR → Haiku → Sonnet (port from `planilhador/extracao/`)
- [ ] Cache layer: Redis `cache_key = (chat_id, message_id, versao_prompt)` TTL 30d
- [ ] Rate limiting: per-user (10/min) + global (100/min) token bucket on Anthropic
- [ ] Circuit breaker: `pybreaker` on Anthropic client (fail_max=5, reset=60s)
- [ ] Materialization workers: idempotent upsert, 1 tx/bet, emits `ApostaCriada`
- [ ] Batch metrics: duration, throughput, cache hit, cost/bet, revisões in Prometheus
...
**Acceptance**: `docker compose up -d redis && celery -A workers.celery_app worker -Q extraction,materialization -l info` starts without errors

---

### Issue 2: Extraction Worker — Core Pipeline (OCR → Haiku → Sonnet)
...
**Acceptance**: Unit test with mocked Anthropic returns structured `ExtracaoBilhete`; conferences run
...
**Acceptance**: Re-processing same image → cache hit, no Anthropic call, metric increments
...
**Acceptance**: Burst test 200 concurrent requests → queued, not rejected; per-user isolation verified
...
**Acceptance**: Simulate 5xx errors → breaker opens, metric shows `state=1`, requests queued
...
**Acceptance**: Duplicate re-send → no duplicate rows, `atualizada_em` updated; grave conference → `revisao_pendente` created
...
**Acceptance**: Metrics exposed at `/metrics`; dashboard shows live data during batch run
...
**Acceptance**: Extension sends bilhete → appears in painel within 30s; duplicate → no-op
...
**Acceptance**: CLI runs, enqueues correct tasks, cost estimate shown

---

### Issue 10: Integration Test — Full Pipeline (Upload → Extraction → Materialization → Painel)
...
**Acceptance**: `pytest tests/integration/test_full_pipeline.py -v` passes; all assertions met
...
```mermaid
graph TD
    W1[Week 1 Complete] --> 1[Redis + Celery Infra]
    1 --> 2[Extraction Worker]
    1 --> 3[Cache Layer]
    1 --> 4[Rate Limiting]
    1 --> 5[Circuit Breaker]
    2 --> 6[Materialization Worker]
    3 --> 2
    4 --> 2
    5 --> 2
    6 --> 7[Batch Metrics]
    6 --> 8[Coleta Casa Integration]
    6 --> 9[Reprocess CLI]
...
| Day | Issues | Notes |
|-----|--------|-------|
| 1 | 1 | Redis + Celery infra (foundation for rest) |
| 2 | 2, 3 | Extraction worker + cache (core pipeline) |
| 3 | 4, 5 | Rate limiting + circuit breaker (resilience) |
| 4 | 6 | Materialization worker (writes to DB) |
| 5 | 7, 8 | Metrics + Coleta casa integration |
| 6 | 9 | Reprocess CLI |
| 7 | 10 | Integration test + validation |

---

## Ready to Create in GitHub?

Run the script (after creating it) to create all 10 issues + 1 milestone.