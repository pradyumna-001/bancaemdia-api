#!/usr/bin/env bash
# Create Week 3 Milestone + 11 Issues in bancaemdia-api repo
# Usage: chmod +x create_week3_issues.sh && ./create_week3_issues.sh
...
MILESTONE_NUMBER=$(gh api repos/$REPO/milestones \
  -f title="$MILESTONE" \
  -f description="JWT auth, Primary/Replica router, all REST endpoints (/upload, /apostas, /caixa, /revisao, /coleta, /painel), full OTel observability, rate limiting, alerting." \
  -f due_on="$(date -d '+21 days' -u +%Y-%m-%dT%H:%M:%SZ)" \
  --jq '.number')
...
create_issue \
  "[Week 3] Authentication — JWT RS256 + JWKS + RLS Integration" \
...
## Acceptance
\`curl -H \"Authorization: Bearer <valid>\" /api/v1/apostas\` → 200 with user's data only"

# Issue 2: Database Router — Primary/Replica Routing Middleware
create_issue \
  "[Week 3] Database Router — Primary/Replica Routing Middleware" \
...
create_issue \
  "[Week 3] Upload Endpoint — Telegram Export Ingestion" \
...
## Acceptance
Upload 10-message export → 202 response; webhook fires; bets appear in DB; status shows completed"

# Issue 4: Aposta CRUD Endpoints — Full Lifecycle
create_issue \
  "[Week 3] Aposta CRUD Endpoints — Full Lifecycle" \
...
## Acceptance
All CRUD operations work; RLS enforced; events appended; projection updates"

# Issue 5: Caixa Endpoints — Depósitos, Saques, Transferências
create_issue \
  "[Week 3] Caixa Endpoints — Depósitos, Saques, Transferências" \
...
## Acceptance
Movimentos created atomically; balances correct; temporal queries work"

# Issue 6: Revisão Pendente Endpoints — Review Queue
create_issue \
  "[Week 3] Revisão Pendente Endpoints — Review Queue" \
...
## Acceptance
Review queue visible; resolution creates correct aposta or discards; events logged"

# Issue 7: Painel Endpoint — Dashboard Aggregates (MV-Backed)
create_issue \
  "[Week 3] Painel Endpoint — Dashboard Aggregates (MV-Backed)" \
...
## Acceptance
Painel loads < 500ms (P95); data < 30s stale; badge shows correct lag"

# Issue 8: Observability Stack — OTel + Structlog + Prometheus + Health
create_issue \
  "[Week 3] Observability Stack — OTel + Structlog + Prometheus + Health" \
...
create_issue \
  "[Week 3] Rate Limiting — slowapi by usuario_id" \
...
## Acceptance
Load test 200 req/min per user → 429 after limit; headers present; WAF rules attached"

# Issue 10: Alerting — CloudWatch Alarms + SNS → Slack
create_issue \
  "[Week 3] Alerting — CloudWatch Alarms + SNS → Slack" \
...
## Acceptance
Inject failure → alarm fires in < 5 min; Slack notification received; runbook linked"

# Issue 11: Contract Tests — API Schema Validation
create_issue \
  "[Week 3] Contract Tests — API Schema Validation" \
...
## Acceptance
\`pytest tests/contract/ -v\` passes; PR with breaking change fails CI"

echo ""
echo "🎉 All 11 issues + 1 milestone created!"
