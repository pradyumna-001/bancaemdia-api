...
def _gravar_relatorio(
    path: Path, resumo: Conferencia, *, staging_gate: bool, phase1_gate: bool = False
) -> None:
    report = {
        "source": "conferir_numeros.py",
        "environment": os.environ.get("APP_ENV", "unknown"),
        "checked_at": datetime.now(UTC).isoformat(),
        "mode": "phase1-gate" if phase1_gate else "staging-gate" if staging_gate else "all",
...
def main() -> int:
    ...
    gate.add_argument("--staging-gate", action="store_true", help="Require 5 banks and 16k bets")
    gate.add_argument("--phase1-gate", action="store_true", help="Local Phase 1 dataset gate")
    ...
    if args.phase1_gate and (not args.todos or os.environ.get("APP_ENV") != "testing"):
        parser.error("--phase1-gate requires --todos and APP_ENV=testing")
    if (args.staging_gate or args.phase1_gate) and not re.fullmatch(
        r"[0-9a-f]{40}", os.environ.get("RELEASE_SHA", "")
    ):
        parser.error("a gate requires a 40-character RELEASE_SHA")
    ...
    if args.todos:
        ...
        if args.json_out:
            _gravar_relatorio(
                args.json_out, resumo, staging_gate=args.staging_gate, phase1_gate=args.phase1_gate
            )
        structlog.get_logger().info("conferencia_resumo", **asdict(resumo))
        return int(not resumo.passou(staging_gate=args.staging_gate or args.phase1_gate))
    try:
