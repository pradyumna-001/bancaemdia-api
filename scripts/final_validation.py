...
def _integrity(manifest: dict[str, Any], base: Path, sha: str) -> Check:
    ...
    passed = (
        ...
        and data.get("environment") == "staging"
        and data.get("mode") == "staging-gate"
...
def _load(manifest: dict[str, Any], base: Path, sha: str) -> Check:
    ...
    passed = (
        ...
        and meta.get("environment") == "staging"
...
def _operational(manifest: dict[str, Any], base: Path) -> list[Check]:
    values = _object(manifest.get("checks"))
    checks: list[Check] = []
    for name in SIMPLE_CHECKS:
    ...
    measured: dict[str, tuple[str, Callable[[float], bool]]] = {
        "rds_failover": ("recovery_seconds", lambda value: value < 30),
...
def _cost(manifest: dict[str, Any], base: Path) -> Check:
...
def evaluate(manifest: dict[str, Any], base: Path) -> list[Check]:
    ...
    checks = [
        Check(
            ...
            manifest.get("environment") == "staging" and bool(sha),
            "staging e SHA imutável exigidos",
        ),
        _integrity(manifest, base, sha),
        _contracts(manifest, base, sha),
        _load(manifest, base, sha),
        *_operational(manifest, base),
        _cost(manifest, base),
    ]
    return checks
